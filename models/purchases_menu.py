...
class PurchasesMenu(models.Model):
    ...
    purchases_menu_ids = fields.One2many('purchases.menu.lines', 'purchases_menu_id',
                                         string="Purchases Menu Lines")
...
class PurchasesMenuLines(models.Model):
    _name = "purchases.menu.lines"
    _inherit = ["mail.thread", "mail.activity.mixin"]
    _description = "Quotations & Purchase Lines"

    purchases_menu_id = fields.Many2one('purchases.menu', string="Line id")
    ...
    contact_type = fields.Selection([
        ('customer', 'Customer'),
        ('supplier', 'Supplier'),
    ], compute='_compute_contact_type', readonly=True)
    price = fields.Float(compute="_compute_price", string="Price")

    @api.depends('product_name_id')
    def _compute_contact_id_domain(self):
        obj = self.env['approved.products'].search([('id', '=', self.product_name_id.id)])
        lines = obj['contacts_ids']
        domain = []
        for line in lines:
            domain.append(line.display_name)
    ...
    def _compute_contact_type(self):
        obj = self.env['approved.products'].search([('product_name', '=', self.product_name_id.id)])
        lines = obj['contacts_ids']
        record = lines.search([('name', '=', self.contact_id.name)])
        for rec in self:
            rec.contact_type = record.contact_type
